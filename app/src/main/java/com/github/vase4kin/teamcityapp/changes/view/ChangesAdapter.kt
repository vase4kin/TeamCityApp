...
class ChangesAdapter(viewHolderFactories: Map<Int, ViewHolderFactory<ChangesDataModel>>) : BaseLoadMoreAdapter<ChangesDataModel>(viewHolderFactories) {
     ...
     * {@inheritDoc}
     */
    override fun onBindViewHolder(holder: BaseViewHolder<ChangesDataModel>, position: Int) {
        super.onBindViewHolder(holder, position)
        // Find the way how to make it through DI
        if (holder is ChangesViewHolder) {
            holder.mContainer.setOnClickListener { onChangeClickListener?.onClick(dataModel.getChange(position)) }
        }
    }
}
